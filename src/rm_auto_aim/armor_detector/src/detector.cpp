...
std::vector<Armor> Detector::detect(const cv::Mat & input)
{
  ...
  // RCLCPP_INFO(rclcpp::get_logger("find lights"), "find lights: %ld", lights_.size());
  armors_ = matchLights(lights_);
  // RCLCPP_INFO(rclcpp::get_logger("match lights"), "match lights: %ld", armors_.size());
  ...
  for (const auto & contour : contours) {
    ...
    if (isLight(light)) {
      ...
      if (  // Avoid assertion failed
        ...
        // TODO： PCA for corner correction
        {
          ...
          for(auto &pt : contour){
            cv::Point2d vec(pt.x - cntr.x, pt.y - cntr.y);
            projections.push_back(vec.x * eigen_vecs[0].x + vec.y * eigen_vecs[0].y);
          }
          light.cor_top = contour[std::max_element(projections.begin(), projections.end()) - projections.begin()];
          light.cor_bottom = contour[std::min_element(projections.begin(), projections.end()) - projections.begin()];
        }
...
void Detector::drawResults(cv::Mat & img)
{
  ...
  for (const auto & light : lights_) {
    ...
    // TODO:corresponding corners
    cv::line(img, light.cor_top, light.cor_bottom, line_color, 1);
  }
